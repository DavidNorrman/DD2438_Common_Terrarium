...
namespace Assets.Scripts
{
    ...
    public class Creature : MonoBehaviour
    {
        ...
        {
            get => currentEnergy;
            set { currentEnergy = Mathf.Min(MaxEnergy, value); }
        }
        ...
        void Update()
        {
            ...
            if (Energy <= 0)
            {
                ...
                this.gameObject.GetComponent<CreatureAI>().AddReward(-1f);
                this.gameObject.GetComponent<CreatureAI>().EndMe();
            }
        ...
        public void Eat(GameObject food)
        {
            ...
            //Destroy(food);
            if (CreatureRegime == Regime.CARNIVORE)
            {
                var herbAI = food.GetComponent<CreatureAI>();
                herbAI.AddReward(-1f);
                herbAI.EndMe();
            }
            else
        ...
        public bool Reproduce()
        {
            ...
            Energy -= EnergyManager.ReproductionCost(this);
            // Children inherit parents instincts
            baby.GetComponent<CreatureAI>().foodInstinct = this.GetComponent<CreatureAI>().foodInstinct;
            return true;
        }
        ...
        Vector3 getClosestFreePoint(Vector3 point)
        {
            ...
            while (closestPoint.magnitude == float.PositiveInfinity)
            {
                ...
                {
                    case 0:
                        testPoint = point + new Vector3(0, 10f, radius);
                        break;
                    case 1:
                        testPoint = point + new Vector3(0, 10f, -radius);
                        break;
                    case 2:
                        testPoint = point + new Vector3(radius, 10f, 0);
                        break;
                    case 3:
                        testPoint = point + new Vector3(-radius, 10f, 0);
                        radius = radius + 1f;
